...
def plot(I660, Ex660, Ey660, Ez660, P660, I785, Ex785, Ey785, Ez785, P785):
    ...
    ax1.plot(phi_deg, I660, color='black', label='660nm, {}mW, Circ.'.format(P660*1e3))
    ax1.plot(phi_deg, I785, color='red', label='785nm, {}mW, Lin.'.format(P785*1e3))
    ax1.set_xlabel(r'$\phi[\text{deg}]$')
    ax1.set_ylabel(r'I')
    ax1.legend(loc='right')

    ax2.plot(phi_deg, np.abs(Ex660)**2, color='black', label='660nm, {}mW, Circ.'.format(P660*1e3))
    ax2.plot(phi_deg, np.abs(Ex785)**2, color='red', label='785nm, {}mW, Lin.'.format(P785*1e3))
    ax2.set_xlabel(r'$\phi[\text{deg}]$')
    ax2.set_ylabel(r'$|E_{x}|^{2}$')
    ax2.legend(loc='right')

    ax3.plot(phi_deg, np.abs(Ey660)**2, color='black', label='660nm, {}mW, Circ.'.format(P660*1e3))
    ax3.plot(phi_deg, np.abs(Ey785)**2, color='red', label='785nm, {}mW, Lin.'.format(P785*1e3))
    ax3.set_xlabel(r'$\phi[\text{deg}]$')
    ax3.set_ylabel(r'$|E_{y}|^{2}$')
    ax3.legend(loc='right')

    ax4.plot(phi_deg, np.abs(Ez660)**2, color='black', label='660nm, {}mW, Circ.'.format(P660*1e3))
    ax4.plot(phi_deg, np.abs(Ez785)**2, color='red', label='785nm, {}mW, Lin.'.format(P785*1e3))
